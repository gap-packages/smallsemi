...
function(sizes, positions)
  return EnumeratorOfSmallSemigroupsByIds([sizes], [positions]);
end);
...
function(sizes, positions)
return EnumeratorOfSmallSemigroupsByIdsNC([sizes],
 [positions]);
end);
...
function(arg)
arg:=SMALLSEMI_STRIP_ARG(arg);
return EnumeratorOfSmallSemigroups(arg);
end);
...
if not SMALLSEMI_ValidateArgs( arg ) then
    Error("argument is incorrect");
    #JDM could just call SMALLSEMI_ValidateArgs and have errors from there
fi;

...
## $Id: properties.gi 264 2010-07-06 11:12:45Z andreas $
...
InstallOtherMethod(IsCompletelySimpleSemigroup, "for a small semigroup", true,
[IsSmallSemigroup], 0, 
function(M)
return IsSimpleSemigroup(M);
end);
...
function( S )
    ...
    # 3-nilpotent semigroups of size 8
...
InstallMethod(IsZeroSemigroup, "for a small semigroup", true, 
[IsSemigroup and HasMultiplicationTable], 0,
function(s)
...
###########################################################################
# change to MinimalGeneratingSet
# split in two methods, second one for abitrary semigroups
# use fact that entries not in the table are generators
InstallMethod(MinimalGeneratingSet,"for a small semigroup",[IsSmallSemigroup],
function(s)
    local subsets, id, pos, entries, gens, subset, k;

    id:=IdSmallSemigroup(s);
    pos:=STORED_INFO(id[1], "MinimalGeneratingSet");

    if not pos=fail then 
        pos:=pos[id[2]];
        pos:=EnumeratorSorted(Combinations([1..id[1]]))[pos];
        return AsSSortedList(s){pos};
    fi;
    ...
    # translate integers to semigroup elements
    entries := AsSSortedList(s){entries};
    gens := AsSSortedList(s){gens};
    
    # for nilpotent semigroups the non-entries are a generating set
    # AD is this test worth the effort? (maybe use HasIsNilpotentSemigroup)
    ...
    for k in [ Maximum(0,1-Length(gens))..Length(entries) ] do
        ...
        for subset in subsets do
            if Size(Semigroup(Concatenation(gens,subset)))=Size(s) then 
                return Concatenation(gens,subset);
            fi;
        od;
    od;
end);
...
function( S )
    ...
    # the generators of a nilpotent semigroups are precisely the elements
